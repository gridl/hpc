...
#sudo bash -c 'echo "//rfdclusterst.file.core.windows.net/rfd /home/mnt cifs vers=3.0,username=rfdclusterst,password=,dir_mode=0777,file_mode=0777,serverino" >> /etc/fstab'
...
sudo cp contrib/init.d/pbs_mom /home/mnt >> /tmp/azuredeploy.log.$$
...
sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS sh -c 'sudo mount -t cifs //rfdstorage.file.core.windows.net/rfd /home/mnt -o vers=3.0,username=rfdstorage,password=7Q+/+S8t/P2NNkIxp3zT15E4kZRdufcDldjCzpRB+Z2QPjgyThbPedHMX8++/mhOdc8nkfJ8Zt/3wVzzT72a1A==,dir_mode=0777,file_mode=0777,serverino'"
sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' |  sudo -kS sh -c 'cp /home/mnt/pbs_mom /etc/init.d/pbs_mom'"
sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' |  sudo -kS sh -c 'chkconfig --add pbs_mom'"
...
#ssh to trust master
   sudo -u rfdmaster sh -c "sshpass -p 'RFDCkjlysqGfhjkm1' ssh-copy-id $WORKER_NAME$i"
sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -iS sh -c 'sudo -u rfdmaster ssh-keygen -f /home/rfdmaster/.ssh/id_rsa -t rsa -N \"\" '"
sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -iS sh -c 'sudo -u rfdmaster sshpass -p '$ADMIN_PASSWORD' ssh-copy-id rfdmaster@master'"
...
#install torque
  ...
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS /etc/init.d/pbs_mom start"
